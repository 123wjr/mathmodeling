...
- `[CONFIRMED]` 使用预注册 30 seeds；每个 seed 轮流留出 5 个压力场景之一，只用其余 4 场景选择编组，形成 `30 x 5 x 2=300` 条策略折级记录。留出场景不进入过滤、裕量计算或 MILP 目标。
- `[CONFIRMED]` 参考策略为原 `INTERVAL_RISK + balanced`；严格鲁棒策略先剔除在任一训练场景触发强制拒绝的候选组，再用带辅助变量的 MILP 最大化所选组最坏归一化安全裕量。所有原阈值保持不变。
- `[RESULT][UPDATEABLE]` 参考策略每折平均选中 5.667 组，严格鲁棒策略为 1.160 组；配对差 -4.507 组，90% seed-bootstrap 区间 `[-5.020,-3.980]`。
- `[RESULT][UPDATEABLE]` 严格鲁棒策略相对参考策略的最坏安全裕量提高 0.332，90% 区间 `[0.181,0.496]`；但稳定组/8 组目标比例下降 0.078，区间 `[-0.106,-0.050]`。
- `[RESULT][UPDATEABLE]` 已选组拒绝率差为 +0.028，90% 区间 `[-0.080,0.134]`，跨 0；不能称严格鲁棒策略降低拒绝率。
- 结论是可证伪的负结果：在当前合成机制和离散压力集下，硬过滤式最坏情形优化改善最坏裕量，却付出显著可信产能代价，未证明样本外拒绝率下降。
